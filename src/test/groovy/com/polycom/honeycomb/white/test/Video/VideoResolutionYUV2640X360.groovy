...
@Title(" VideoResolutionYUV2640x360 Case")
class VideoResolutionYUV2640X360 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionYUV2640X360.class)
    ...
    @Unroll
    def "Format -- YUV2_Resolution -- 640x360_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteWin.startVideo("YUY2", "640x360", (int) rate)
        whiteMac.startVideo("YUY2", "640x360", (int) rate)
        sleep(5000)
        ...
        then:
        noExceptionThrown()
        resolutionWin == "640x360"
        resolutionMac == "640x360"
        compareRate(actualRatesWin, rate)
        compareRate(actualRatesMac, rate)

        where:
        rate << whiteWin.getSupportedFrameRates( "YUY2", "640x360")
    }
}
