...
@Title ("VideoResolutionMJPEG3840x2160")
class VideoResolutionMJPEG3840X2160 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionMJPEG3840X2160.class)
    ...
    @Unroll
    def "Format -- MJPEG_Resolution -- 3840x2160_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteWin.startVideo("MJPG", "3840x2160", (int) rate)
        whiteMac.startVideo("MJPG", "3840x2160", (int) rate)
        sleep(5000)
        ...
        then:
        noExceptionThrown()
        resolutionWin == "3840x2160"
        resolutionMac == "3840x2160"
        compareRate(actualRatesWin, rate)
        compareRate(actualRatesMac, rate)

        where:
        rate << whiteWin.getSupportedFrameRates( "MJPG", "3840x2160")
    }
}