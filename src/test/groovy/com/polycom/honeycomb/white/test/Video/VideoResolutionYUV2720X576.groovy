...
@Title("VideoResolutionYUV2720x576")
class VideoResolutionYUV2720X576 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionYUV2720X576.class)
    static int index = -1
    ...
    @Unroll
    def "Format -- YUV2_Resolution -- 720x576_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteWin.startVideo("YUY2", "720x576", (int) rate)
        whiteMac.startVideo("YUY2", "720x576", (int) rate)
        sleep(5000)
        ...
        where:
        rate << whiteWin.getSupportedFrameRates( "YUY2", "720x576")
    }
}
