...
@Title ("USB30_MAC_NV12_800x600")
class USB30_MAC_NV12_800x600 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(USB30_MAC_NV12_800x600.class)
    ...
    @Unroll
    def "Format -- NV12_Resolution -- 800x600_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteMac.startVideo("NV12", "800x600", (int) rate)
        sleep(5000)
        ...
        where:
        rate << whiteMac.getSupportedFrameRates( "NV12", "800x600")
    }
}
