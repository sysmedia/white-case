...
@Title ("USB30_MAC_NV12_720x480")
class USB30_MAC_NV12_720x480 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(USB30_MAC_NV12_720x480.class)
    ...
    @Unroll
    def "Format -- NV12_Resolution -- 720x480_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteMac.startVideo("NV12", "720x480", (int) rate)
        sleep(5000)
        ...
        where:
        rate << whiteMac.getSupportedFrameRates( "NV12", "720x480")
    }
}
