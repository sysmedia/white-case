...
@Title ("VideoResolutionMJPEG1024X576")
class VideoResolutionMJPEG1024X576 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionMJPEG1024X576.class)
    ...
    @Unroll
    def "Format -- MJPEG_Resolution -- 1024x576_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteWin.startVideo("MJPG", "1024x576", (int) rate)
        whiteMac.startVideo("MJPG", "1024x576", (int) rate)
        sleep(5000)
        ...
        where:
        rate << whiteWin.getSupportedFrameRates( "MJPG", "1024x576")
    }
}
