...
@Title ("VideoResolutionMJPEG720X480")
class VideoResolutionMJPEG720X480 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionMJPEG720X480.class)
    ...
    @Unroll
    def "Format -- MJPEG_Resolution -- 720x480_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteWin.startVideo("MJPG", "720x480", (int) rate)
        whiteMac.startVideo("MJPG", "720x480", (int) rate)
        sleep(5000)
        ...
        where:
        rate << whiteWin.getSupportedFrameRates( "MJPG", "720x480")
    }
}
