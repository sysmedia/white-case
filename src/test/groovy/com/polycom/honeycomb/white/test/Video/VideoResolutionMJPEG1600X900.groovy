...
@Title ("VideoResolutionMJPEG1600X900")
class VideoResolutionMJPEG1600X900 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionMJPEG1600X900.class)
    ...
    @Unroll
    def "Format -- MJPEG_Resolution -- 1600x900_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteWin.startVideo("MJPG", "1600x900", (int) rate)
        whiteMac.startVideo("MJPG", "1600x900", (int) rate)
        sleep(5000)
        ...
        where:
        rate << whiteWin.getSupportedFrameRates( "MJPG", "1600x900")
    }
}
