...
@Title("VideoResolutionYUV2640x480 case")
class VideoResolutionYUV2640X480 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionYUV2640X480.class)
    ...
    @Unroll
    def "Format -- YUV2_Resolution -- 640x480_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteWin.startVideo("YUY2", "640x480", (int) rate)
        whiteMac.startVideo("YUY2", "640x480", (int) rate)
        sleep(5000)
        ...
        then:
        noExceptionThrown()
        resolutionWin == "640x480"
        resolutionMac == "640x480"
        compareRate(actualRatesWin, rate)
        compareRate(actualRatesMac, rate)

        where:
        rate << whiteWin.getSupportedFrameRates( "YUY2", "640x480")
    }
}
