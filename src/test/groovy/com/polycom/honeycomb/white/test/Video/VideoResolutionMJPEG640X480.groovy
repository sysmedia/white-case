...
@Title("VideoResolutionMJPEG640X480")
class VideoResolutionMJPEG640X480 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionMJPEG640X480.class)
    ...
    @Unroll
    def "Format -- MJPEG_Resolution -- 640x480_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteWin.startVideo("MJPG", "640x480", (int) rate)
        whiteMac.startVideo("MJPG", "640x480", (int) rate)
        sleep(5000)
        ...
        where:
        rate << whiteWin.getSupportedFrameRates( "MJPG", "640x480")
    }
}
