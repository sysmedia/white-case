...
@Title("VideoResolutionMJPEG960x540")
class VideoResolutionMJPEG960X540 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionMJPEG960X540.class)
    ...
    @Unroll
    def "Format -- MJPEG_Resolution -- 960x540_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteWin.startVideo("MJPG", "960x540", (int) rate)
        whiteMac.startVideo("MJPG", "960x540", (int) rate)
        sleep(5000)
        ...
        where:
        rate << whiteWin.getSupportedFrameRates( "MJPG", "960x540")
    }
}
