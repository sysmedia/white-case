 ...
 */
@Title("VideoResolutionYUV2480x270 case")
class VideoResolutionYUV2480X270 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionYUV2480X270.class)
    ...
    @Unroll
    def "Format -- YUV2_Resolution -- 480x270_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteWin.startVideo("YUY2", "480x270", (int) rate)
        whiteMac.startVideo("YUY2", "480x270", (int) rate)
        sleep(5000)
        ...
        then:
        noExceptionThrown()
        resolutionWin == "480x270"
        resolutionMac == "480x270"
        compareRate(actualRatesWin, rate)
        compareRate(actualRatesMac, rate)

        where:
        rate << whiteWin.getSupportedFrameRates( "YUY2", "480x270")
    }
}
