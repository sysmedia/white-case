...
@Title("VideoResolutionMJPEG800x600")
class VideoResolutionMJPEG800X600 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionMJPEG800X600.class)
    ...
    @Unroll
    def "Format -- MJPEG_Resolution -- 800x600_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteWin.startVideo("MJPG", "800x600", (int) rate)
        whiteMac.startVideo("MJPG", "800x600", (int) rate)
        sleep(5000)
        ...
        where:
        rate << whiteWin.getSupportedFrameRates( "MJPG", "800x600")
    }
}
