...
@Title ("VideoResolutionMJPEG1280X720")
class VideoResolutionMJPEG1280X720 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionMJPEG1280X720.class)
    ...
    @Unroll
    def "Format -- MJPEG_Resolution -- 1280x720_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteWin.startVideo("MJPG", "1280x720", (int) rate)
        whiteMac.startVideo("MJPG", "1280x720", (int) rate)
        sleep(5000)
        ...
        where:
        rate << whiteWin.getSupportedFrameRates( "MJPG", "1280x720")
    }
}