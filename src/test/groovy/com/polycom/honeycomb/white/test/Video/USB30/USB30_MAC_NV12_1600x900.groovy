...
@Title ("USB30_MAC_NV12_1600x900")
class USB30_MAC_NV12_1600x900 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(USB30_MAC_NV12_1600x900.class)
    ...
    @Unroll
    def "Format -- NV12_Resolution -- 1600x900_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteMac.startVideo("NV12", "1600x900", (int) rate)
        sleep(5000)
        ...
        where:
        rate << whiteMac.getSupportedFrameRates( "NV12", "1600x900")
    }
}
