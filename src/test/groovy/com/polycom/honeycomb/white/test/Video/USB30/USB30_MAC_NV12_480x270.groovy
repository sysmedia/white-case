...
@Title ("USB30_MAC_NV12_480x270")
class USB30_MAC_NV12_480x270 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(USB30_MAC_NV12_480x270.class)
    ...
    @Unroll
    def "Format -- NV12_Resolution -- 480x270_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteMac.startVideo("NV12", "480x270", (int) rate)
        sleep(5000)
        ...
        where:
        rate << whiteMac.getSupportedFrameRates( "NV12", "480x270")
    }
}
