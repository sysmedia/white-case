...
@Title("VideoResolutionMJPEG640X360")
class VideoResolutionMJPEG640X360 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionMJPEG640X360.class)
    ...
    @Unroll
    def "Format -- MJPEG_Resolution -- 640x360_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteWin.startVideo("MJPG", "640x360", (int) rate)
        whiteMac.startVideo("MJPG", "640x360", (int) rate)
        sleep(5000)
        ...
        where:
        rate << whiteWin.getSupportedFrameRates( "MJPG", "640x360")
    }
}
