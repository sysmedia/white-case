...
@Title ("VideoResolutionYUV21600X900 Case")
class VideoResolutionYUV21600X900 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionYUV21600X900.class)
    ...
    @Unroll
    def "Format -- YUV2_Resolution -- 1600x900_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteWin.startVideo("YUY2", "1600x900", (int) rate)
        whiteMac.startVideo("YUY2", "1600x900", (int) rate)
        sleep(5000)
        ...
        then:
        noExceptionThrown()
        resolutionWin == "1600x900"
        resolutionMac == "1600x900"
        compareRate(actualRatesWin, rate)
        compareRate(actualRatesMac, rate)

        where:
        rate << whiteWin.getSupportedFrameRates( "YUY2", "1600x900")
    }
}
