...
@Title("VideoResolutionMJPEG480X270")
class VideoResolutionMJPEG480X270 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionMJPEG480X270.class)
    ...
    @Unroll
    def "Format -- MJPEG_Resolution -- 480x270_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteWin.startVideo("MJPG", "480x270", (int) rate)
        whiteMac.startVideo("MJPG", "480x270", (int) rate)
        sleep(5000)
        ...
        where:
        rate << whiteWin.getSupportedFrameRates( "MJPG", "480x270")
    }
}
