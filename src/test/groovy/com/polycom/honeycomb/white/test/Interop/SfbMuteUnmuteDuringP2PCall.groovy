...
class SfbMuteUnmuteDuringP2PCall extends SystemTestSpec {
    private static final Logger logger = LoggerFactory.getLogger(SfbMuteUnmuteDuringP2PCall.class)
    ...
    def "mute during call"(){
        when:
        lyncCaller.muteAudio()
        sleep(2000)
        logger.info(LogUtils.imageHtml(white.screenshot()))
        sleep(2000)
//        def status = white.getStatusInfo()

        then:
        assert lyncCaller.isAudioMuted()
//        assert status.muteSet == "mute"

    }

    def "un-mute during call"(){
        when:
        lyncCaller.unmuteAudio()
        sleep(2000)
        logger.info(LogUtils.imageHtml(white.screenshot()))
        sleep(2000)
//        def status = white.getStatusInfo()
