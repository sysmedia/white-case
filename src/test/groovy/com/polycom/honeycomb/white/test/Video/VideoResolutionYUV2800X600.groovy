 ...
 */
@Title("VideoResolutionYUV2800x600 case")
class VideoResolutionYUV2800X600 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionYUV2800X600.class)
    ...
    @Unroll
    def "Format -- YUV2_Resolution -- 800x600_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteWin.startVideo("YUY2", "800x600", (int) rate)
        whiteMac.startVideo("YUY2", "800x600", (int) rate)
        sleep(5000)
        ...
        then:
        noExceptionThrown()
        resolutionWin == "800x600"
        resolutionMac == "800x600"
        compareRate(actualRatesWin, rate)
        compareRate(actualRatesMac, rate)

        where:
        rate << whiteWin.getSupportedFrameRates( "YUY2", "800x600")
    }
}
