...
@Title ("VideoResolutionYUV2352X288 Case")
class VideoResolutionYUV2352X288 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionYUV2352X288.class)
    ...
    @Unroll
    def "Format -- YUV2_Resolution -- 352x288_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteWin.startVideo("YUY2", "352x288", (int) rate)
        whiteMac.startVideo("YUY2", "352x288", (int) rate)
        sleep(5000)
