...
@Title ("USB30_MAC_NV12_640x360")
class USB30_MAC_NV12_640x360 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(USB30_MAC_NV12_640x360.class)
    ...
    @Unroll
    def "Format -- NV12_Resolution -- 640x360_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteMac.startVideo("NV12", "640x360", (int) rate)
        sleep(5000)
        ...
        where:
        rate << whiteMac.getSupportedFrameRates( "NV12", "640x360")
    }
}
