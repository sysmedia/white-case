...
@Title ("VideoResolutionNV121920X1080 Case")
class VideoResolutionNV121920X1080 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionNV121920X1080.class)
    static int index = -1
    ...
    @Unroll
    def "Format -- NV12_Resolution -- 1920x1080_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f",rate))
        white.startVideo("NV12", "1920x1080", (int) rate)
        sleep(5000)
        ...
        where:
        rate << white.getSupportedFrameRates("NV12", "1920x1080")
