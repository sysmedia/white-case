...
@Title ("VideoResolutionYUV21024X576 Case")
class VideoResolutionYUV21024X576 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionYUV21024X576.class)
    ...
    @Unroll
    def "Format -- YUV2_Resolution -- 1024x576_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f", rate))
        whiteWin.startVideo("YUY2", "1024x576", (int) rate)
        whiteMac.startVideo("YUY2", "1024x576", (int) rate)
        sleep(5000)
        ...
        then:
        noExceptionThrown()
        resolutionWin == "1024x576"
        resolutionMac == "1024x576"
        compareRate(actualRatesWin, rate)
        compareRate(actualRatesMac, rate)

        where:
        rate << whiteWin.getSupportedFrameRates( "YUY2", "1024x576")
    }
}
