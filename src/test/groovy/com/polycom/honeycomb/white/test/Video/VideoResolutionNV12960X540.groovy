...
@Title("VideoResolutionNV12960x540 case")
class VideoResolutionNV12960X540 extends SystemTestSpec{
    static final Logger LOGGER = LoggerFactory.getLogger(VideoResolutionNV12960X540.class)
    static int index = -1
   // static final String whiteName = "Polycom White Video"
    ...
    @Unroll
    def "Format -- NV12_Resolution -- 960x540_Rate -- #rate" (Float rate) {
        when:
        LOGGER.info(String.format("Start White by rate %.1f",rate))
        white.startVideo("NV12", "960x540", (int) rate)
        sleep(5000)
        ...
        where:
        rate << white.getSupportedFrameRates("NV12", "960x540")
